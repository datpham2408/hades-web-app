 const DETAIL_TEXT = {
  vi: {
    pageTitle: "Chi tiết sản phẩm",
    home: "Trang chủ",
    category: "Danh mục",
    product: "Sản phẩm",
    descriptionTitle: "Mô tả sản phẩm",
    infoTitle: "Thông tin",
    colorLabel: "Màu sắc",
    sizeLabel: "Kích thước",
    addToCart: "THÊM VÀO GIỎ",
    buyNow: "MUA NGAY",
    black: "Đen",
    signature: "Signature",
    note:
      "Người mẫu cao 1m78, mặc size M. Đơn hàng nội thành dự kiến giao trong 2-4 ngày làm việc. Kiểm tra kỹ thông tin size trước khi đặt hàng.",
    addedToCart: "Đã thêm size {size} vào giỏ hàng.",
    categoryFallback: "DANH MỤC",
  },
  en: {
    pageTitle: "Product Detail",
    home: "Home",
    category: "Category",
    product: "Product",
    descriptionTitle: "Product description",
    infoTitle: "Details",
    colorLabel: "Color",
    sizeLabel: "Size",
    addToCart: "ADD TO CART",
    buyNow: "BUY NOW",
    black: "Black",
    signature: "Signature",
    note:
      "The model is 1m78 tall and wears size M. Urban orders are estimated to arrive within 2-4 business days. Please check the size details carefully before placing your order.",
    addedToCart: "Added size {size} to cart.",
    categoryFallback: "CATEGORY",
  },
};

const DETAIL_CONTENT = {
  vi: {
    descriptions: {
      tops:
        "Thiết kế mang tinh thần streetwear hiện đại với phom dáng vừa vặn, chất liệu dày dặn và bề mặt hoàn thiện sạch sẽ cho cảm giác cao cấp.",
      bottoms:
        "Phom dáng rộng vừa, tập trung vào chuyển động tự nhiên và sự thoải mái khi phối cùng áo oversized hoặc outerwear đặc trưng.",
      outerwear:
        "Lớp ngoài tối giản nhưng sắc nét, được xử lý bề mặt để giữ cảm giác cứng cáp, phù hợp với phong cách layering của streetwear hiện đại.",
      underwear:
        "Thiết kế gọn gàng, chất liệu mềm, tập trung vào sự linh hoạt hằng ngày nhưng vẫn giữ tinh thần thương hiệu mạnh mẽ.",
      bags:
        "Phụ kiện mang tinh thần utilitarian với bề mặt chắc tay, cấu trúc gọn và phù hợp cho nhịp sống đô thị.",
      accessories:
        "Chi tiết nhỏ nhưng có độ nhận diện cao, giúp hoàn thiện outfit theo tinh thần tối giản và cá tính.",
      sale:
        "Phiên bản ưu đãi với phom dáng đặc trưng của thương hiệu, giữ nguyên tinh thần thiết kế nhưng có mức giá dễ tiếp cận hơn.",
    },
    info: {
      tops: [
        "Chất liệu cotton dày, bề mặt đứng form.",
        "Đường may sạch, cổ áo giữ phom ổn định.",
        "Phù hợp mặc độc lập hoặc layering.",
      ],
      bottoms: [
        "Phom loose fit thoải mái.",
        "Chất vải bền, dễ phối với nhiều kiểu áo.",
        "Tập trung vào chuyển động và độ rũ đẹp.",
      ],
      outerwear: [
        "Cấu trúc ngoài cứng cáp, dễ layering.",
        "Tông màu tối, tối giản và hiện đại.",
        "Phù hợp outfit streetwear hằng ngày.",
      ],
      underwear: [
        "Chất liệu mềm, độ co giãn tốt.",
        "Thiết kế tối giản, dễ sử dụng mỗi ngày.",
        "Tạo cảm giác gọn gàng và linh hoạt.",
      ],
      bags: [
        "Bề mặt chắc tay, giữ form tốt.",
        "Không gian chứa đồ đủ dùng hằng ngày.",
        "Chi tiết hoàn thiện gọn gàng, hiện đại.",
      ],
      accessories: [
        "Hoàn thiện outfit bằng điểm nhấn nhỏ.",
        "Thiết kế tối giản, dễ phối đồ.",
        "Giữ tinh thần nhận diện thương hiệu.",
      ],
      sale: [
        "Sản phẩm trong nhóm ưu đãi nổi bật.",
        "Thiết kế nguyên bản, giá dễ tiếp cận hơn.",
        "Phù hợp làm điểm bắt đầu cho outfit mới.",
      ],
    },
    categoryLabels: {
      tops: "TOPS",
      bottoms: "BOTTOMS",
      outerwear: "OUTERWEARS",
      underwear: "UNDERWEARS",
      bags: "BAGS",
      accessories: "ACCESSORIES",
      sale: "SALE",
    },
  },
  en: {
    descriptions: {
      tops:
        "Modern streetwear design with a regular fit, substantial fabric, and a clean finish that feels premium.",
      bottoms:
        "A relaxed wide fit focused on natural movement and comfort, pairing well with oversized tops or signature outerwear.",
      outerwear:
        "A minimal yet sharp outer layer with a structured finish, made to support contemporary streetwear layering.",
      underwear:
        "Clean-cut design with soft materials and everyday flexibility, while keeping a strong brand character.",
      bags:
        "Utilitarian accessories with a firm surface, compact structure, and a fit for urban daily carry.",
      accessories:
        "Small pieces with strong recognition that complete the outfit with a minimal, personal edge.",
      sale:
        "A discounted edit with the brand's signature silhouette, keeping the design language while being more accessible.",
    },
    info: {
      tops: [
        "Heavy cotton with a structured surface.",
        "Clean stitching and a stable collar shape.",
        "Works well on its own or layered.",
      ],
      bottoms: [
        "Comfortable loose fit.",
        "Durable fabric that pairs easily with many tops.",
        "Focused on movement and a nice drape.",
      ],
      outerwear: [
        "Structured shell that layers easily.",
        "Dark, minimal, and modern tones.",
        "Ideal for everyday streetwear looks.",
      ],
      underwear: [
        "Soft material with good stretch.",
        "Minimal design for everyday use.",
        "Creates a neat and flexible feel.",
      ],
      bags: [
        "Firm surface that keeps its shape well.",
        "Enough storage space for daily use.",
        "Clean and modern finishing details.",
      ],
      accessories: [
        "Finish the outfit with a small accent.",
        "Minimal design that is easy to style.",
        "Keeps the brand identity sharp.",
      ],
      sale: [
        "Highlights from the current sale selection.",
        "Original design, now at a friendlier price.",
        "A good entry point for a new outfit.",
      ],
    },
    categoryLabels: {
      tops: "TOPS",
      bottoms: "BOTTOMS",
      outerwear: "OUTERWEARS",
      underwear: "UNDERWEARS",
      bags: "BAGS",
      accessories: "ACCESSORIES",
      sale: "SALE",
    },
  },
};

const DETAIL_DEFAULT_SIZE = "M";

let selectedDetailSize = DETAIL_DEFAULT_SIZE;
let currentDetailProduct = null;

function getDetailLanguage() {
  return localStorage.getItem("language") === "en" ? "en" : "vi";
}

function dt(key) {
  return DETAIL_TEXT[getDetailLanguage()][key];
}

function getDetailContent() {
  return DETAIL_CONTENT[getDetailLanguage()];
}

function getDetailElements() {
  return {
    name: document.getElementById("detailName"),
    sku: document.getElementById("detailSku"),
    price: document.getElementById("detailPrice"),
    colorName: document.getElementById("detailColorName"),
    description: document.getElementById("detailDescription"),
    info: document.getElementById("detailInfo"),
    note: document.getElementById("detailNote"),
    breadcrumbName: document.getElementById("detailBreadcrumbName"),
    categoryLink: document.getElementById("detailCategoryLink"),
    thumbsContainer: document.getElementById("detailThumbs"),
    mainImage: document.getElementById("detailMainImage"),
    feedback: document.getElementById("detailFeedback"),
  };
}

function getDetailProduct() {
  const params = new URLSearchParams(window.location.search);
  const productId = Number(params.get("id"));
  return productData.find((product) => product.id === productId) || productData[0];
}

function getDetailImages(product) {
  return [product.imgFront, product.imgBack].filter(Boolean).map(getImageUrl);
}

function getDetailCategoryHref(category) {
  if (category === "accessories") {
    return getPageUrl("accessory.html#product-list");
  }

  return getPageUrl(`${category}.html#product-list`);
}

function renderDetailThumbnails(product, images, elements) {
  if (!elements.thumbsContainer || !elements.mainImage) {
    return;
  }

  elements.mainImage.src = images[0];
  elements.mainImage.alt = product.name;
  elements.thumbsContainer.innerHTML = images
    .map(
      (image, index) => `
        <button
          class="detail-thumb ${index === 0 ? "is-active" : ""}"
          type="button"
          data-image="${image}"
        >
          <img src="${image}" alt="${product.name}" loading="lazy">
        </button>
      `,
    )
    .join("");
}

function renderDetailInfo(category, infoElement) {
  if (!infoElement) {
    return;
  }

  infoElement.innerHTML = (getDetailContent().info[category] || getDetailContent().info.tops)
    .map((item) => `<li>${item}</li>`)
    .join("");
}

function updateDetailText() {
  const elements = getDetailElements();
  const lang = getDetailLanguage();
  const content = getDetailContent();
  document.documentElement.lang = lang;

  if (document.title) {
    document.title = dt("pageTitle");
  }

  if (elements.breadcrumbName && currentDetailProduct) {
    elements.breadcrumbName.textContent = currentDetailProduct.name;
  }

  const breadcrumbLinks = document.querySelectorAll(".detail-breadcrumb a");

  if (breadcrumbLinks[0]) {
    breadcrumbLinks[0].textContent = dt("home");
  }

  if (elements.categoryLink) {
    elements.categoryLink.textContent =
      content.categoryLabels[currentDetailProduct?.category] || dt("categoryFallback");
  }

  const headingEls = document.querySelectorAll(".detail-heading");
  if (headingEls[0]) {
    headingEls[0].textContent = dt("descriptionTitle");
  }
  if (headingEls[1]) {
    headingEls[1].textContent = dt("infoTitle");
  }

  const labels = {
    color: document.querySelector(".detail-color-label"),
    size: document.querySelector(".detail-size-label"),
    add: document.getElementById("detailAddToCart"),
    buy: document.getElementById("detailBuyNow"),
  };

  if (labels.color) {
    labels.color.textContent = dt("colorLabel");
  }

  if (labels.size) {
    labels.size.textContent = dt("sizeLabel");
  }

  if (labels.add) {
    labels.add.textContent = dt("addToCart");
  }

  if (labels.buy) {
    labels.buy.textContent = dt("buyNow");
  }

  if (elements.note) {
    elements.note.textContent = dt("note");
  }
}

function renderDetailProduct() {
  const elements = getDetailElements();
  currentDetailProduct = getDetailProduct();

  if (
    !currentDetailProduct ||
    !elements.name ||
    !elements.sku ||
    !elements.price ||
    !elements.colorName ||
    !elements.description ||
    !elements.info ||
    !elements.note
  ) {
    return;
  }

  const images = getDetailImages(currentDetailProduct);
  const soldOut = isProductSoldOut(currentDetailProduct);

  elements.name.textContent = currentDetailProduct.name;
  elements.sku.textContent = `SKU: HDS-${currentDetailProduct.id}`;
  elements.price.textContent = soldOut ? t("soldOut") : formatPrice(currentDetailProduct.price);
  elements.colorName.textContent = currentDetailProduct.name.includes("BLACK")
    ? dt("black")
    : dt("signature");
  elements.description.textContent =
    getDetailContent().descriptions[currentDetailProduct.category] ||
    getDetailContent().descriptions.tops;
  elements.note.textContent = dt("note");

  if (elements.categoryLink) {
    elements.categoryLink.href = getDetailCategoryHref(currentDetailProduct.category);
  }

  renderDetailInfo(currentDetailProduct.category, elements.info);
  renderDetailThumbnails(currentDetailProduct, images, elements);

  document.getElementById("detailAddToCart").disabled = soldOut;
  document.getElementById("detailBuyNow").disabled = soldOut;

  updateDetailText();
}

function setActiveSize(size) {
  selectedDetailSize = size;

  document.querySelectorAll(".detail-size-btn").forEach((button) => {
    button.classList.toggle("is-active", button.dataset.size === size);
  });
}

function showDetailFeedback(message) {
  const { feedback } = getDetailElements();

  if (feedback) {
    feedback.textContent = message;
  }
}

function saveDetailItemToCart(redirectToCheckout = false) {
  if (!currentDetailProduct || isProductSoldOut(currentDetailProduct)) {
    return;
  }

  const detailItem = buildCartItem(currentDetailProduct, {
    selectedSize: selectedDetailSize,
  });

  if (redirectToCheckout) {
    saveCartStorage([detailItem]);
    window.location.href = getPageUrl("checkout.html");
    return;
  }

  let cart = getCartStorage();
  const existingItem = cart.find(
    (item) =>
      item.id === currentDetailProduct.id && item.selectedSize === selectedDetailSize,
  );

  if (existingItem) {
    existingItem.quantity += 1;
  } else {
    cart = [...cart, detailItem];
  }

  saveCartStorage(cart);
  renderCart();
  updateCartCount();
  openCart();
  showDetailFeedback(dt("addedToCart").replace("{size}", selectedDetailSize));
}

function addDetailProductToCart(event) {
  event?.stopPropagation();
  saveDetailItemToCart(false);
}

function buyDetailProductNow() {
  saveDetailItemToCart(true);
}

function handleDetailClick(event) {
  const thumbButton = event.target.closest(".detail-thumb");
  const sizeButton = event.target.closest(".detail-size-btn");

  if (thumbButton) {
    const { mainImage } = getDetailElements();

    if (mainImage) {
      mainImage.src = thumbButton.dataset.image;
    }

    document.querySelectorAll(".detail-thumb").forEach((button) => {
      button.classList.toggle("is-active", button === thumbButton);
    });
  }

  if (sizeButton) {
    setActiveSize(sizeButton.dataset.size);
  }
}

function initDetailPage() {
  document.addEventListener("click", handleDetailClick);
  document
    .getElementById("detailAddToCart")
    ?.addEventListener("click", addDetailProductToCart);
  document
    .getElementById("detailBuyNow")
    ?.addEventListener("click", buyDetailProductNow);

  renderDetailProduct();
  setActiveSize(selectedDetailSize);

  window.addEventListener("languagechange", () => {
    renderDetailProduct();
    setActiveSize(selectedDetailSize);
  });

  window.addEventListener("storage", (event) => {
    if (event.key !== "language") {
      return;
    }

    renderDetailProduct();
    setActiveSize(selectedDetailSize);
  });
}

initDetailPage();
